...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        if (curr_state[0] + curr_state[1] + curr_state[2] == 15 or
            ...
            curr_state[0] + curr_state[3] + curr_state[6] == 15 or
            curr_state[1] + curr_state[4] + curr_state[7] == 15 or
            curr_state[2] + curr_state[5] + curr_state[8] == 15 or
            curr_state[0] + curr_state[4] + curr_state[8] == 15 or
    ...
    def state_transition(self, curr_state, curr_action):
        ...
        """
        #We need to create new instacne every time, else updating new_state will also update curr_state
        new_state = [val for val in curr_state]
    ...
    def step(self, curr_state, curr_action):
        ...
        terminal_state, state_val = self.is_terminal(new_state)
        ...
        if terminal_state:
            ...
            #Evnironment is taking an action
            agent_actions, env_actions = self.action_space(new_state)
            env_actions = list(env_actions)
            random_index = random.choice(range(len(env_actions)))
            action = env_actions[random_index]
